...
async def parse_and_store(session: AsyncSession) -> int:
    ...
    try:
        ...
        parse_runs_total.labels(status="error").inc()
        parse_duration_seconds.observe(time.monotonic() - started_at)
        return 0

    parsed_vacancies_total.inc(created_total)
    parse_runs_total.labels(status="success").inc()
    parse_duration_seconds.observe(time.monotonic() - started_at)
